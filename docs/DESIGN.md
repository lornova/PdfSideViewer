...
```
MainWindow (frame, menu/toolbar, accelerators, find bar)
...
│   ├── res/                   icons, version resource
│   └── src/
│       ├── main.cpp           wWinMain, message loop, accelerators
│       ├── MainWindow.*       frame, splitter, toolbar, find bar
...
│       ├── search/TextSearch.*
│       └── util/              UTF conversion, JSON settings, misc
└── vendor/
